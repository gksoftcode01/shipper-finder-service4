...
export default defineComponent({
  ...
  setup() {
    ...
    const cargoMsgs: Ref<ICargoMsg[]> = ref([]);
    ...
    const retrieveCargoMsgs = async () => {
      ...
      try {
        ...
        const res = currentSearch.value
          ? await cargoMsgService().search(currentSearch.value, paginationQuery)
          : await cargoMsgService().retrieve(paginationQuery);
        totalItems.value = Number(res.headers['x-total-count']);
        queryCount.value = totalItems.value;
        links.value = dataUtils.parseLinks(res.headers?.link);
        cargoMsgs.value.push(...(res.data ?? []));
      } catch (err) {
    ...
    const prepareRemove = (instance: ICargoMsg) => {
    ...
    const removeCargoMsg = async () => {
      try {
        await cargoMsgService().delete(removeId.value);
        const message = t$('shipperfinderservice4App.cargoMsg.deleted', { param: removeId.value }).toString();
    ...
    watch([cargoMsgs, page], async ([data, page], [_prevData, prevPage]) => {
