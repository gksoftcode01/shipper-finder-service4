  ...
  setup() {
    ...
    const retrieveCargoRequestItems = async () => {
      ...
      try {
        ...
        const res = currentSearch.value
          ? await cargoRequestItemService().search(currentSearch.value, paginationQuery)
          : await cargoRequestItemService().retrieve(paginationQuery);
    ...
    const removeCargoRequestItem = async () => {
      try {
        ...
        const message = t$('shipperfinderservice4App.cargoRequestItem.deleted', { param: removeId.value }).toString();
