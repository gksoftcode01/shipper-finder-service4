...
export default defineComponent({
  ...
  setup() {
    ...
    const retrieveCargoRequests = async () => {
      ...
      try {
        ...
        const res = currentSearch.value
          ? await cargoRequestService().search(currentSearch.value, paginationQuery)
          : await cargoRequestService().retrieve(paginationQuery);
        totalItems.value = Number(res.headers['x-total-count']);
        queryCount.value = totalItems.value;
        links.value = dataUtils.parseLinks(res.headers?.link);
        cargoRequests.value.push(...(res.data ?? []));
      } catch (err) {
    ...
    const search = query => {
      ...
      retrieveCargoRequests();
    };

    const removeId: Ref<number> = ref(null);
    const removeEntity = ref<any>(null);
    const prepareRemove = (instance: ICargoRequest) => {
    ...
    const removeCargoRequest = async () => {
      try {
        await cargoRequestService().delete(removeId.value);
        const message = t$('shipperfinderservice4App.cargoRequest.deleted', { param: removeId.value }).toString();
    ...
    watch([cargoRequests, page], async ([data, page], [_prevData, prevPage]) => {
