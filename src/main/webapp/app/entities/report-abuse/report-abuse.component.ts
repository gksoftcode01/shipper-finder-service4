...
export default defineComponent({
  ...
  setup() {
    ...
    const reportAbuses: Ref<IReportAbuse[]> = ref([]);
    ...
    const retrieveReportAbuses = async () => {
      ...
      try {
        ...
        const res = currentSearch.value
          ? await reportAbuseService().search(currentSearch.value, paginationQuery)
          : await reportAbuseService().retrieve(paginationQuery);
        totalItems.value = Number(res.headers['x-total-count']);
        queryCount.value = totalItems.value;
        links.value = dataUtils.parseLinks(res.headers?.link);
        reportAbuses.value.push(...(res.data ?? []));
      } catch (err) {
    ...
    const search = query => {
      ...
      retrieveReportAbuses();
    };

    const removeId: Ref<number> = ref(null);
    const removeEntity = ref<any>(null);
    const prepareRemove = (instance: IReportAbuse) => {
    ...
    const removeReportAbuse = async () => {
      try {
        await reportAbuseService().delete(removeId.value);
        const message = t$('shipperfinderservice4App.reportAbuse.deleted', { param: removeId.value }).toString();
    ...
    watch([reportAbuses, page], async ([data, page], [_prevData, prevPage]) => {
