  ...
  setup() {
    ...
    const retrieveUnits = async () => {
      ...
      try {
        ...
        const res = currentSearch.value
          ? await unitService().search(currentSearch.value, paginationQuery)
          : await unitService().retrieve(paginationQuery);
    ...
    const removeUnit = async () => {
      try {
        ...
        const message = t$('shipperfinderservice4App.unit.deleted', { param: removeId.value }).toString();
    ...
    watch([units, page], async ([data, page], [_prevData, prevPage]) => {
