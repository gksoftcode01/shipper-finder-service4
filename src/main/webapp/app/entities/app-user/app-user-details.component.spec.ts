import { vitest } from 'vitest';
import { type MountingOptions, shallowMount } from '@vue/test-utils';
import sinon, { type SinonStubbedInstance } from 'sinon';
import { type RouteLocation } from 'vue-router';

import AppUserDetails from './app-user-details.vue';
import AppUserService from './app-user.service';
import AlertService from '@/shared/alert/alert.service';

type AppUserDetailsComponentType = InstanceType<typeof AppUserDetails>;

let route: Partial<RouteLocation>;
const routerGoMock = vitest.fn();

vitest.mock('vue-router', () => ({
  useRoute: () => route,
  useRouter: () => ({ go: routerGoMock }),
}));

const appUserSample = { id: 123 };

describe('Component Tests', () => {
  let alertService: AlertService;

  afterEach(() => {
    vitest.resetAllMocks();
  });

  describe('AppUser Management Detail Component', () => {
    let appUserServiceStub: SinonStubbedInstance<AppUserService>;
    let mountOptions: MountingOptions<AppUserDetailsComponentType>['global'];

    beforeEach(() => {
      route = {};
      appUserServiceStub = sinon.createStubInstance<AppUserService>(AppUserService);

      alertService = new AlertService({
        i18n: { t: vitest.fn() } as any,
        bvToast: {
          toast: vitest.fn(),
        } as any,
      });

      mountOptions = {
        stubs: {
          'font-awesome-icon': true,
          'router-link': true,
        },
        provide: {
          alertService,
          appUserService: () => appUserServiceStub,
        },
      };
    });

    describe('Navigate to details', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        appUserServiceStub.find.resolves(appUserSample);
        route = {
          params: {
            appUserId: `${123}`,
          },
        };
        const wrapper = shallowMount(AppUserDetails, { global: mountOptions });
        const comp = wrapper.vm;
        // WHEN
        await comp.$nextTick();

        // THEN
        expect(comp.appUser).toMatchObject(appUserSample);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        appUserServiceStub.find.resolves(appUserSample);
        const wrapper = shallowMount(AppUserDetails, { global: mountOptions });
        const comp = wrapper.vm;
        await comp.$nextTick();

        comp.previousState();
        await comp.$nextTick();

        expect(routerGoMock).toHaveBeenCalledWith(-1);
      });
    });
  });
});
