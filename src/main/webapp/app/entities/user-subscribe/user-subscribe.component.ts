...
export default defineComponent({
  ...
  setup() {
    ...
    const userSubscribes: Ref<IUserSubscribe[]> = ref([]);
    ...
    const retrieveUserSubscribes = async () => {
      ...
      try {
        ...
        const res = currentSearch.value
          ? await userSubscribeService().search(currentSearch.value, paginationQuery)
          : await userSubscribeService().retrieve(paginationQuery);
        totalItems.value = Number(res.headers['x-total-count']);
        queryCount.value = totalItems.value;
        links.value = dataUtils.parseLinks(res.headers?.link);
        userSubscribes.value.push(...(res.data ?? []));
      } catch (err) {
    ...
    const search = query => {
      ...
      retrieveUserSubscribes();
    };

    const removeId: Ref<number> = ref(null);
    const removeEntity = ref<any>(null);
    const prepareRemove = (instance: IUserSubscribe) => {
    ...
    const removeUserSubscribe = async () => {
      try {
        await userSubscribeService().delete(removeId.value);
        const message = t$('shipperfinderservice4App.userSubscribe.deleted', { param: removeId.value }).toString();
    ...
    watch([userSubscribes, page], async ([data, page], [_prevData, prevPage]) => {
