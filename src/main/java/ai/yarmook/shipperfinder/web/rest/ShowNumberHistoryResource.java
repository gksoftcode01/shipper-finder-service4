...
public class ShowNumberHistoryResource {
    ...
    /**
     * {@code SEARCH  /show-number-histories/_search?query=:query} : search for the showNumberHistory corresponding
     * to the query.
     *
     * @param query the query of the showNumberHistory search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<ShowNumberHistoryDTO>> searchShowNumberHistories(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to search for a page of ShowNumberHistories for query {}", query);
        try {
            Page<ShowNumberHistoryDTO> page = showNumberHistoryService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
